...
file_location = r'C:\Users\AA\Desktop\Mytrading\Trading\XBX Quant\data\basic-trading-data\stock_data'
...
for fp in sorted(file_list)[:300]:
    ...
    #  合并数据
    all_data = all_data._append(df, ignore_index=True)  # 注意此时若一下子导入很多文件，可能会内存溢出
...
# 将数据存入hdf文件中
all_data.to_hdf(
    r'C:\Users\AA\Desktop\Mytrading\Trading\XBX Quant\data\basic-trading-data\a_stock.h5',
    key='all_data',
    mode='w'
)
